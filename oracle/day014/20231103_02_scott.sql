...
--※ <20231103_01_scott(qlsql).sql> 의 프로시저 관련 실습을 진행하기 위한 준비
...
( HAKBUN    NUMBER      -- CHECK~!!! --*원래 학번은 VARCHAR2이지만, 빠른 진행을 위해 NUMBER로 임시 생성* 
, NAME      VARCHAR2(40)
, KOR       NUMBER(3)
, ENG       NUMBER(3)
, MAT       NUMBER(3)
...
--※ 여기에서 추가한 컬럼에 대한 항목은 프로시저 실습을 쉽게 진행할 수 있도록 추가한 항목일 뿐
--   실제 테이블 구조에 적합하지도, 바람직하지도 않은 내용이다~!!!  CHECK~!!!

--==>> 기존 테이브의 데이터를 기반으로 쿼리문을 활용하여 얻을 수 있는 데이터는
--     절대 테이블에 추가로 구성(테이블 내에 컬럼화) 하지 않는다.

-- *테이블의 특정한 값으로 얻어낼 수 있는 값은 절대로 컬럼화시키면 안됨(바람직하지 않음)*  
-- *오히려 무결성을 해치는 문제가 생김*  
-- *(EX. 컬럼화시키면 안되는것: 나이, 총점, 평균, 등급등...
-- *이유: 쿼리를 통해 나이가 몇살인지 알아내는 구조로 만들어야 업데이트 시킬 필요가 없어짐)* 
