...
```
## 6.1. <20231103_01_scott(qlsql).sql> 의 프로시저 관련 실습을 진행하기 위한 준비
-- 실습 테이블 생성
``` SQL
...
```
>-- *테이블 2개*  
-- *1) 입력받는 데이터에대해 예측가능한 컬럼 -> 코드화
-- *EX) 관계: 안사람, 아내, 와이프...
-- *2) '사원가족' 테이블에서 '주민번호'가 P.K면 사내에서 결혼 했을 경우 등록되지 않음
...
-- <EXEC PRC_STUDENT_INSERT('moon', '문정환', '010-1111-1111', '제주도 서귀포시');>  
