...
=====================================*/
/*
○ 인터페이스(Interface)란			→ JDK 1.6버전부터
	완전히 미완성된 채로 남겨져						//** 추상화가 극강으로 심함
	인터페이스 안에 존재하는 그 어떤 메소드도
	몸체(정의부)가 없기 때문에 사실상 실행 부분이 존재하지 않는다.
	클래스를 위한 템플릿으로써의 기능을 수행하는	//** 템플릿으로써의 기능: XXX양식
	추상 클래스의 한 종류이다.

○ 인터페이스는 클래스와 달리 다중 상속이 가능하며
	인터페이스 자체도 상속된다.
	기존의 c++ 에서 지원되는 다중 상속이
...
○ 특징
	...
	- 인터페이스를 『implements』 한 클래스는
		인터페이스의 모든 메소드를 Overrinding 해야 한다.
...
public class Test117
{
	public static void main(String[] args)
	{
		...
		//ob.write();
		//--==>> 에러발생(컴파일 에러): cannot find symbol

		//○ 다운 캐스팅
		((DemoImpl)ob).write();
		//--==>> 클래스에 정의된 메소드...

		System.out.print(Demo.PI);	//-- static이기 때문에(PI는 상수화된 클래스변수)

		System.out.print(Demo.a);	//-- 자동으로 static

		//Demo.a = 300;				//-- final이기 떄문에
		//--==>> 에러 발생(컴파일 에러)
	}
}